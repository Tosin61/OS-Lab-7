...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
   ...
   node_t *c = l->head;
    ...
    while (case)
    {
        if ((!c->next || newblk->start <= c->next->blk->start) && newblk->start > c->blk->start)
        {
            node_t *temp = c->next;
            c->next = n;
            n->next = temp;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   node_t *c = l->head;
    ...
    if (!c || compareSize(blocksize, c->blk))
    {
        list_add_to_front(l, newblk);
        return;
    }
    ...
    while (c)
    {
        if ((!c->next || compareSize(blocksize, c->next->blk)) && !compareSize(blocksize, c->blk))
        {
            node_t *temp = c->next;
            c->next = n;
            n->next = temp;
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
   node_t *prev = l->head;
    ...
    while (prev && current && list_length(l) != 0)
    {
